...
int start();
int checkEdge();
int opened(int X, int Y);
int checkEdge(int X, int Y);
int mineFlagged(int X, int Y);
int nonEdgeProbUpdate(int X,int Y,int currentBoxValue);
int edgeProbUpdate(int X,int Y,int currentBoxValue);
int addProb(int X, int Y, int boxProb);
int mineFlagged(int X,int Y);
...
int addProb(int X, int Y, int boxProb){ //adds the probability of surronding boxes of being a bomb
...
int checkEdge(int X, int Y){ //returns 1 if Edge given
	if(X == 0 || X == 7 || Y == 0 || Y == 7){
	return 1;
	}
	return 0;
