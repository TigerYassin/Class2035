...
void* insertItem(HashTable* hashTable, unsigned int key, void* value) {
  ...
  int bucketIndex = hashTable -> hash(key);//find the appropriate bucket 
  ...
  HashTableEntry *thisNode = createHashTableEntry(key,value); //creating new entry
...
void* removeItem(HashTable* hashTable, unsigned int key) {
  int bucketIndex = hashTable -> hash(key);//get the appropriate bucket 
  HashTableEntry *thisNode = hashTable -> buckets[bucketIndex];
  if(thisNode && thisNode -> key == key)
  {
    void* removedEntryVal = thisNode -> value;
    hashTable -> buckets[bucketIndex] = thisNode ->next;//moving our head pointer to the next value 
    free(thisNode);//FREE THE HEAD
  ...
  while(thisNode && thisNode -> next)
  {
    ...
    {
      HashTableEntry* entryToRemove = thisNode -> next; //saving the variable 
      void* removedEntryVal = entryToRemove -> value;
      thisNode -> next = thisNode -> next -> next;
      free(entryToRemove);
      
      return removedEntryVal;//free the value that we wish to be removed 
    }
...
//MUST get a specific item and be able to efficiently delete said item 
void deleteItem(HashTable* hashTable, unsigned int key) {
  ...
  if (thisNode && thisNode ->key == key)//MUST DELETE THE HEAD
  {
    hashTable -> buckets[bucketIndex] = thisNode -> next; //updating the head 
    free(thisNode -> value);
    free(thisNode);
    return;
  }
  ...
  while(thisNode && thisNode -> next)
  {
    if(thisNode -> next -> key == key)
    {
      HashTableEntry* entryToDelete = thisNode -> next;//keep track of my entry 
      thisNode -> next = thisNode -> next -> next;

      free(entryToDelete -> value); //freeing my value 
  
      free(entryToDelete);//then freeing the entry 
      return;
    }
    
    thisNode = thisNode -> next; //getting our next entry 
  }
}
